...
# Version 8/21/09 by Andrew Harrington
# Use Python 2.6 string format method for __str__ for graphics objects.  
...
#           - simplification of Harrington's getLastMouse/clearMouse
...
class Image(GraphicsObject):
    ...
    def __str__(self):
        return 'Image at {0}'.format(self.anchor) 
...
def test():
    ...
    for pt in p.getPoints():
        s = s + "({0:.1f},{1:.1f}) ".format(pt.getX(), pt.getY())
    t.setText(e.getText())
    e.setFill("green")
    e.setText("Spam!")
    e.move(2,0)
